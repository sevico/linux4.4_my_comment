...
IPPROTO_IP实现了自己的bind函数，IPPROTO_TCP和IPPROTO_UDP都使用AF_INET通用的函数，即这个inet_bind。*/
	...
	if (addr->sin_family != AF_INET) {
		 ...
		 */
		 /* 本来要求地址的协议族要与sock相同，必须为AF_INET，但是这里有个兼容性问题。允许协议族为AF_UNSPEC并且地址为INADDR_ANY的任意地址 */
		err = -EAFNOSUPPORT;
	 ...
	 */
	err = -EADDRNOTAVAIL;
	/*sysctl_ip_nonlocal_bind系统控制开关，允许bind非本地IP; inet->freebind为一个socket选项，
	允许该socket bind任意IP；在上面这些变量均不成立时，指定地址又不是任意的本地地址INADDR_ANY，地址类型又不是本地地址类型，多播或广播时，则bind失败。*/
...
static int __init inet_init(void)
{
	...
	//注册AF_INET的各种协议，如UDP、TCP等
	/*
	inet_add_proto-col面向的是底层接口，而inet_register_protosw面向的是上层应用，
	所以将其分为了两个结构。
	*/
